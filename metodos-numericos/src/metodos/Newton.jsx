...
const makePanZoomHandlers = (range, setRange, width, padL, padR) => {
  ...
  const clientXToX = (svg, clientX, xMin, xMax) => {
    ...
    let px = clientX - rect.left; // posición dentro del SVG
    px = Math.max(padL, Math.min(width - padR, px)); // clamp
  ...
  const onMouseMove = (e) => {
    ...
    const { xMin, xMax } = range;
    const dxX = -dxPx * (xMax - xMin) / innerW; // pan
...
export default function Newton() {
  ...
  const buildCompiled = (expr) => {
    const t = expr.trim();
    if (!t) return null;
    try { return math.compile(normalizeExpr(t)); } catch { return null; }
  };
  const getDecimals = () => {
  ...
  const handleCalculate = (e) => {
    ...
    if (tol <= 0) { setErrorMsg("La tolerancia debe ser positiva."); return; }
    if (maxIter <= 0) { setErrorMsg("El número de iteraciones debe ser > 0."); return; }

    x0 = roundTo(x0);
    const cF = buildCompiled(fxInput);
    const cDf = buildCompiled(dfxInput);
    if (!cF || !cDf) { setErrorMsg("No se pudo interpretar f(x) o f'(x)."); return; }

    const f  = (x) => { try { const r = cF.evaluate({x});  return Number.isFinite(r) ? r : NaN; } catch { return NaN; } };
    const df = (x) => { try { const r = cDf.evaluate({x}); return Number.isFinite(r) ? r : NaN; } catch { return NaN; } };

    const newRows = [];
    let xn = x0, ok = false, bad = false;
    ...
    try {
      for (let n = 1; n <= maxIter; n++) {
        ...
        if (!Number.isFinite(fxn) || !Number.isFinite(dfxn)) {
          setErrorMsg("No se pudo evaluar f o f' en alguna iteración."); bad = true; break;
        }
        if (dfxn === 0) {
          setErrorMsg("Apareció f'(xₙ)=0. Newton no puede continuar."); bad = true; break;
        }
        const xNext = roundTo(xn - fxn/dfxn);
        const error = roundTo(Math.abs(xNext - xn));
        newRows.push({ n, xn, fxn, dfxn, xNext, error });
        if (error < tol) { ok = true; break; }
        xn = xNext;
      }
    } catch {
      setErrorMsg("Error inesperado durante las iteraciones."); bad = true;
    }
    setRows(newRows);
    if (!newRows.length || bad) return;
    const last = newRows[newRows.length - 1];
    setMessage(ok ? `Se encontró una aproximación: x ≈ ${formatNumber(last.xNext)}` :
                    "Se alcanzó el máximo de iteraciones sin cumplir tolerancia.");
  };

  const handleClear = () => {
    setFxInput(""); setDfxInput(""); setX0Input("");
  ...
  const graphBase = useMemo(() => {
    ...
    let xMin, xMax;
    if (Number.isFinite(x0)) { xMin = x0 - 2; xMax = x0 + 2; } else { xMin = -5; xMax = 5; }
  ...
  // rango interactivo en X (se recalibra cuando cambia la base)
  ...
  const buildTicks = (min, max, count=6) => {
    ...
    const ticks=[]; for(let i=0;i<=count;i++){ ticks.push(min + i*(max-min)/count); } return ticks;
  };
  ...
  // construir la vista general con rangoMain y Y automático
  const buildMainView = (rangeX) => {
    ...
    else { const m = (yMax - yMin) * 0.15; yMin -= m; yMax += m; }
  ...
  useEffect(()=>{ setRangeA(autoRangeFor(first3)); }, [rows.length]);   // re-auto al recalcular
  ...
  const buildTangentView = (items, rangeX) => {
    ...
    pts.forEach(p=>{ yMin=Math.min(yMin,p.y); yMax=Math.max(yMax,p.y); });
    items.forEach(r=>{
      const m=r.dfxn, c=r.fxn - m*r.xn;
      const ya=m*xMin+c, yb=m*xMax+c;
      yMin=Math.min(yMin, ya, yb); yMax=Math.max(yMax, ya, yb);
    });
    if (!Number.isFinite(yMin) || !Number.isFinite(yMax) || yMin===yMax){ yMin=-1; yMax=1; }
    else { const m=(yMax-yMin)*0.15; yMin-=m; yMax+=m; }
  ...
  return (
    <div className="bisection-grid">
      ...
      <div className="bisection-results">
        ...
        <div className="bisection-table-wrapper">
          ...
          {rows.length === 0 ? (
            ...
            <table className="bisection-table">
              <thead>
                <tr>
                  <th>n</th><th>xₙ</th><th>f(xₙ)</th><th>f&apos;(xₙ)</th><th>xₙ₊₁</th><th>|xₙ₊₁ - xₙ|</th>
                </tr>
              </thead>
              <tbody>
                {rows.map(r=>(
              ...
              >
                ...
                {v.xTicks.map((t,i)=>(
                  <g key={`xt${i}`}>
                    ...
                    <text x={t.X} y={height-6} fontSize="9" textAnchor="middle" fill="#374151">{t.x.toFixed(2)}</text>
                  </g>
                ))}
                {v.yTicks.map((t,i)=>(
                  <g key={`yt${i}`}>
                    <line x1={v.yAxisX-3} x2={v.yAxisX+3} y1={t.Y} y2={t.Y} stroke="#6b7280"/>
                    <text x={padL-6} y={t.Y+3} fontSize="9" textAnchor="end" fill="#374151">{t.y.toFixed(2)}</text>
                  </g>
                ))}
                ...
                {lastRow && (
                  <line
                    x1={v.xTo(lastRow.xNext)} x2={v.xTo(lastRow.xNext)}
        ...
        {/* ---- Tangentes: Primeras 3 (con zoom, pan y escala) ---- */}
        {rows.length > 0 && (
          <div className="graph-card">
        ...
        {/* ---- Tangentes: Últimas 3 (con zoom, pan y escala) ---- */}
        {rows.length > 0 && (
          <div className="graph-card">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
