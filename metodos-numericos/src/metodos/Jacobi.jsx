...
export default function Jacobi() {
  ...
  const [iterInput, setIterInput] = useState("23");

  // matriz aumentada de ejemplo
  ...
  const isDiagonallyDominant = (A) => {
    ...
    for (let i = 0; i < A.length; i++) {
      ...
      if (diag > sum) atLeastOneStrict = true;
    }

    return atLeastOneStrict || true;
  };
  ...
  const jacobi = (A, b, xInit, maxIter, tol) => {
    ...
    for (let iter = 1; iter <= maxIter; iter++) {
      ...
      for (let i = 0; i < n; i++) {
        const err = Math.abs(xNew[i] - xPrev[i]);
        errs.push(err);
      ...
      if (maxErr < tol) {
        return { rows: history, solution: [...xNew], converged: true };
      }

      xPrev = [...xNew];
    }

    return { rows: history, solution: [...xPrev], converged: false };
  };
  ...
  const handleCalculate = (e) => {
    ...
    const maxIter = clampInt(parseInt(iterInput, 10) || 1, 1, 300);
  ...
  return (
    ...
    >
      <div className="bisection-form" style={{ display: "grid", gap: 16 }}>
        <Card>
          <h3 style={{ marginTop: 0, marginBottom: 8 }}>Método de Jacobi</h3>
          ...
          <Card>
            ...
            <FormRow label="Tamaño de la matriz (n)">
              <input
                ...
                value={sizeInput}
                onChange={(e) =>
                  setSizeInput(String(clampInt(parseInt(e.target.value || "2", 10), 2, 10)))
                }
              />
            </FormRow>
                    ...
                    return (
                      <tr key={row.iter}>
                        ...
                        <td
                          style={
                            isLast
                              ? { background: "#fecaca", color: "#991b1b", fontWeight: 800 }
                              : undefined
                          }
                        >
                          {fmt(row.maxErr)}
