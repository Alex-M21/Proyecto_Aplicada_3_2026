...
export default function PosicionFalsa1() {
  ...
  const [xPrevInput, setXPrevInput] = useState("1");
  ...
  const formatNumber = (value) =>
    Number.isFinite(value) ? value.toFixed(getDecimals()) : "NaN";
  ...
  const handleCalculate = (e) => {
    ...
    if (!Number.isFinite(fPrev0) || !Number.isFinite(fCurr0)) {
      setErrorMsg(
        "No se pudo evaluar f(x) en los extremos iniciales. Revisa dominio."
      );
      return;
    }
    ...
    if (fPrev0 * fCurr0 > 0) {
      setErrorMsg(
        "f(xₙ₋₁) y f(xₙ) tienen el mismo signo. Se requiere cambio de signo en [xₙ₋₁, xₙ]."
      );
      return;
    }
    ...
    try {
      for (let n = 1; n <= maxIter; n++) {
        ...
        if (denom === 0) {
          setErrorMsg(
            "En alguna iteración f(xₙ) - f(xₙ₋₁) = 0. No se puede continuar."
          );
          hadError = true;
          break;
        }
        ...
        if (!Number.isFinite(fNext)) {
          setErrorMsg(
            "No se pudo evaluar f(xₙ₊₁) en alguna iteración. Revisa dominio."
          );
          hadError = true;
          break;
        }
    ...
    <div className="bisection-grid">
      <div className="bisection-form">
        ...
        <p className="bisection-hint">
          Ingresa <strong>f(x)</strong> y un intervalo inicial{" "}
          <strong>[xₙ₋₁, xₙ]</strong> con cambio de signo. También acepta{" "}
            ...
            <>
              ...
              >
                ...
                {rowView && (
                  <>
                    <circle
                      cx={graphData.xTo(rowView.xPrevRaw)}
                      cy={graphData.yTo(rowView.fPrevRaw)}
                      r="4"
                      fill="#f59e0b"
                    />
                    <circle
                      cx={graphData.xTo(rowView.xCurrRaw)}
                      cy={graphData.yTo(rowView.fCurrRaw)}
                      r="4"
                      fill="#ef4444"
                    />
                    ...
                    <circle
                      cx={graphData.xTo(rowView.xNextRaw)}
                      cy={graphData.xAxisY}
                      r="4.4"
                      fill="#10b981"
                    />
                    ...
                    <text
                      x={graphData.xTo(rowView.xNextRaw) + 6}
              ...
              <p className="bisection-hint" style={{ marginTop: 8 }}>
                Rueda del mouse: zoom • Arrastrar: mover • Slider: ver secante por iteración •
                Puntos negros: historial de aproximaciones xₙ₊₁
              </p>
            </>
          )}
        </div>
