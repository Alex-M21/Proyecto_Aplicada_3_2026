...
export default function GaussSeidel() {
  ...
  const [matrix, setMatrix] = useState(
    ejemplosGaussSeidel[3].Aumentada.map((row) => [...row])
  );
  const [x0, setX0] = useState([...ejemplosGaussSeidel[3].x0]);

  const [tolInput, setTolInput] = useState(ejemplosGaussSeidel[3].tolerancia);
  const [iterInput, setIterInput] = useState(ejemplosGaussSeidel[3].iteraciones);
  const [decimalsInput, setDecimalsInput] = useState(
    ejemplosGaussSeidel[3].decimales
  );
  ...
  const loadSize = (size) => {
    ...
    if (ejemplosGaussSeidel[size]) {
      const ejemplo = ejemplosGaussSeidel[size];
  ...
  const gaussSeidel = (A, b, xInit, maxIter, tol) => {
    ...
    for (let iter = 1; iter <= maxIter; iter++) {
      ...
      const errors = x.map((value, i) => Math.abs(value - previous[i]));
  ...
  const exportCSV = () => {
    ...
    link.href = url;
    link.download = `gauss_seidel_matriz_${n}x${n}.csv`;
    link.click();
  ...
  const downloadChartPNG = () => {
    ...
    image.onload = () => {
      ...
      link.href = pngUrl;
      link.download = `grafica_error_gauss_seidel_matriz_${n}x${n}.png`;
      link.click();
    ...
    <div className="bisection-form">
      ...
      <form onSubmit={handleCalculate}>
        <div className="method-section">
          ...
          <p className="bisection-hint">
            {ejemplosGaussSeidel[n]
              ? `Ejemplo cargado: ${ejemplosGaussSeidel[n].descripcion}`
